...
trait SqlMockTrait
{
    ...
    {
        $pdo = $this->mockSelect('select `test_models`.*, (select count(*) from `relation_models` where `test_models`.`id` = `relation_models`.`test_model_id`) as `relation_count` from `test_models` where `test_models`.`deleted_at` is not null and `id` = ?', [
            1
        ], $selectResult);
    ...
    {
        $pdo = $this->mockSelect('select `test_models`.*, (select count(*) from `relation_models` where `test_models`.`id` = `relation_models`.`test_model_id`) as `relation_count` from `test_models` where `test_models`.`deleted_at` is not null and `id` in (?, ?, ?)', [
            1,
            2,
            3
        ], $selectResult);
    ...
    protected function mockCreate(array $selectResult): void
    {
        ...
        $this->mockInsert('insert into `test_models` (`name`, `updated_at`, `created_at`) values (?, ?, ?)', [
            'test_name',
            Carbon::now(),
            Carbon::now()
        ], 1, $pdo);

        $this->mockSelect('select * from `test_models` where `id` = ? limit 1', [
            1
        ], $selectResult, $pdo);
    ...
    protected function mockUpdate(array $selectResult): void
    {
        ...
        $this->mockUpdateSqlQuery('update `test_models` set `name` = ?, `test_models`.`updated_at` = ? where `id` = ?', [
            'test_name',
            Carbon::now(),
            1
        ], null, $pdo);
    ...
    {
        $pdo = $this->mockSelect('select exists(select `test_models`.*, (select count(*) from `relation_models` where `test_models`.`id` = `relation_models`.`test_model_id`) as `relation_count` from `test_models` where `test_models`.`deleted_at` is not null and `id` = ?) as `exists`', [
            1
        ], [
            ['exists' => true]
        ]);

        $this->mockSelect('select `test_models`.*, (select count(*) from `relation_models` where `test_models`.`id` = `relation_models`.`test_model_id`) as `relation_count` from `test_models` where `test_models`.`deleted_at` is not null and `id` = ? limit 1', [
            1
        ], $selectResult, $pdo);
    ...
    protected function mockUpdateOrCreateEntityDoesntExist(array $selectResult): void
    {
        ...
        $this->mockInsert('insert into `test_models` (`name`, `id`, `updated_at`, `created_at`) values (?, ?, ?, ?)', [
            'test_name',
            1,
            Carbon::now(),
            Carbon::now()
    ...
    {
        $pdo = $this->mockSelect('select count(*) as aggregate from `test_models` where `test_models`.`deleted_at` is not null', [], [
            ['aggregate' => 1]
        ]);
    ...
    {
        $pdo = $this->mockSelect('select count(*) as aggregate from `test_models`', [], [
            ['aggregate' => 1]
        ]);
    ...
    {
        $this->mockSelect("select exists(select * from `{$table}` where `{$keyField}` <> ? and `email` in (?, ?)) as `exists`", [
            1,
            'mail@mail.com',
            'mail@mail.net'
        ], [
